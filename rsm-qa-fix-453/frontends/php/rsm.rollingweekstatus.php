...
foreach ($DB['SERVERS'] as $key => $value) {
	 ...
	 */
	if ($data['registrar_mode'] || ($data['filter_cctld_group'] || $data['filter_gtld_group']
		...
		foreach ($tld_groups as $tld_group) {
			switch ($tld_group['name']) {
				...
				case RSM_CC_TLD_GROUP:
					...
					if ($data['registrar_mode'] || $data['filter_cctld_group']) {
				...
				case RSM_G_TLD_GROUP:
					...
					if ($data['registrar_mode'] || $data['filter_gtld_group']) {
						$included_groupids[$tld_group['groupid']] = $tld_group['groupid'];
					}
					break;
				...
				case RSM_OTHER_TLD_GROUP:
					...
					if ($data['registrar_mode'] || $data['filter_othertld_group']) {
						$included_groupids[$tld_group['groupid']] = $tld_group['groupid'];
					}
					break;
				...
				case RSM_TEST_GROUP:
					...
					if ($data['registrar_mode'] || $data['filter_test_group']) {
						$included_groupids[$tld_group['groupid']] = $tld_group['groupid'];
					}
					break;
			}
		...
		if ($data['filter_registrar_family'] !== '') {
			...
			$registrar_family_matching_hosts = zbx_objectValues($registrar_family_matching_hosts[0]['hosts'], 'hostid');
		...
		$where_host = $where_host ? ' AND ('.implode(' OR ', $where_host).')' : '';
		...
		if ($db_tlds) {
			$hostids = [];
			$extra_fields = $data['registrar_mode']
