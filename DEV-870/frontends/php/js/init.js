...
// range control example plugin.
jQuery.fn.rangeControl = function(options) {
	var $=jQuery;
	var tmpl = $('<div class="range-control"><div class="range-control-track"/><div class="range-control-progress"/><datalist/><div class="range-control-thumb"/><input type="range"/></div>');
	...
	return $(this).each(function(_, input) {
		var listid=(new Date()).getTime().toString(34),
			step=options.step||1,
			min=options.min,
			max=options.max,
			range=options.range||getRange(min, max, step),
			interval=Math.ceil((max-min)/step)*step,
			$input=$(input),
			$control=tmpl.clone(),
			$range=$control.find('[type=range]').attr({
			'list': listid,
			'min': min,
			'max': max,
			'step': step,
