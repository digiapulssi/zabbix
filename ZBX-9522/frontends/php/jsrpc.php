...
switch ($data['method']) {
	...
	case 'notifications.unmute':
		...
		updateMessageSettings($msgsettings);
		break;
	// TODO could be only close method with explicid IDS sent - frontend would send all ids then.
	// this might ensure that unseen notification is not closed before receiving.
	case 'notifications.close_all':
		$msgsettings = getMessageSettings();
		switch (strtolower($data['params']['caption'])) {
			case 'events':
				$msgsettings['last.clock'] = (int) $data['params']['time'] + 1;
	...
	case 'notifications.get':
		...
		foreach ($events as $number => $event) {
			...
			if (count($usedTriggers) < 15) {
				if (!isset($usedTriggers[$event['objectid']])) {
					...
					$number = (string) $event['eventid'];
					$result[$number] = [
						'uid' => $number,
						'ttl' => $event['clock'] + $msgsettings['timeout'] - time(),
						/* 'ttl' => abs($event['clock'] + $msgsettings['timeout'] - time()), // TODO remove demo */
						'priority' => $priority,
						'sound' => $sound,
						'file' => $sound,
						'severity_style' => getSeverityStyle($trigger['priority'], $event['value'] == TRIGGER_VALUE_TRUE),
		...
		array_multisort($sortClock, SORT_DESC, $sortEvent, SORT_DESC, $result);
