...
class CHttpTest extends CApiService {
	...
	protected function validateCreate(array &$httptests) {
		$api_input_rules = ['type' => API_OBJECTS, 'flags' => API_NOT_EMPTY | API_NORMALIZE, 'uniq' => [['hostid', 'name']], 'fields' => [
			...
			'variables' =>			['type' => API_STRING_UTF8, 'length' => DB::getFieldLength('httptest', 'variables')],
			'headers' =>			['type' => API_STRING_UTF8, 'length' => DB::getFieldLength('httptest', 'headers')],
			...
			'steps' =>				['type' => API_OBJECTS, 'flags' => API_REQUIRED | API_NOT_EMPTY, 'uniq' => [['name'], ['no']], 'fields' => [
				...
				'posts' =>				['type' => API_STRING_UTF8, 'length' => DB::getFieldLength('httpstep', 'posts')],
				'variables' =>			['type' => API_STRING_UTF8, 'length' => DB::getFieldLength('httpstep', 'variables')],
				'headers' =>			['type' => API_STRING_UTF8, 'length' => DB::getFieldLength('httpstep', 'headers')],
	...
	protected function validateUpdate(array &$httptests, array &$db_httptests = null) {
		...
		$db_httptests = $this->get([
			'output' => ['httptestid', 'hostid', 'name', 'applicationid', 'delay', 'retries', 'agent', 'http_proxy',
				'variables', 'headers', 'status', 'authentication', 'http_user', 'http_password', 'verify_peer',
				'verify_host', 'ssl_cert_file', 'ssl_key_file', 'ssl_key_password', 'templateid'
			],
		...
		$db_httpsteps = DB::select('httpstep', [
			'output' => ['httpstepid', 'httptestid', 'name', 'no', 'url', 'timeout', 'posts', 'required',
				'status_codes', 'variables', 'follow_redirects', 'retrieve_mode', 'headers'
			],
