...
if (isset($_REQUEST['favobj'])) {
	if('filter' == $_REQUEST['favobj']){
		CProfile::update('web.rsm.screens.filter.state', getRequest('favstate'), PROFILE_TYPE_INT);
	}
}
...
$data = array(
	'filter_year' => getRequest('filter_year'),
	'filter_month' => getRequest('filter_month'),
	'type' => getRequest('type'),
	'item_key' => getRequest('item_key')
);
...
if (!$tld) {
	show_error_message(_s('No permissions to referred TLD "%1$s" or it does not exist!', getRequest('tld')));
	require_once dirname(__FILE__).'/include/page_footer.php';
	exit;
}
...
$item = API::Item()->get(array(
	...
	'filter' => array(
		'key_' => getRequest('item_key')
	)
));

if (!$item) {
	show_error_message(_s('Item with key "%1$s" not exist on TLD!', getRequest('item_key')));
	require_once dirname(__FILE__).'/include/page_footer.php';
	exit;
}
...
switch ($data['item']['key_']) {
	case RSM_SLV_DNS_DOWNTIME:
		if (getRequest('type') == RSM_SLA_SCREEN_TYPE_GRAPH_1) {
	...
	case RSM_SLV_DNS_TCP_RTT_PFAILED:
		if (getRequest('type') == RSM_SLA_SCREEN_TYPE_GRAPH_1 || getRequest('type') == RSM_SLA_SCREEN_TYPE_GRAPH_2) {
			if (getRequest('type') == RSM_SLA_SCREEN_TYPE_GRAPH_1) {
	...
	case RSM_SLV_DNS_UDP_RTT_PFAILED:
		if (getRequest('type') == RSM_SLA_SCREEN_TYPE_GRAPH_1 || getRequest('type') == RSM_SLA_SCREEN_TYPE_GRAPH_2) {
			...
			// Failed tests and maximum expected number of tests per month.
			$item_keys = array(RSM_SLV_DNS_UDP_RTT_FAILED, RSM_SLV_DNS_UDP_RTT_MAX);
			$items = API::Item()->get(array(
				'output' => array('itemid', 'key_', 'value_type'),
				'hostids' => $data['tld']['hostid'],
				'filter' => array(
			...
			foreach ($items as $item) {
				if ($item['key_'] == RSM_SLV_DNS_UDP_RTT_FAILED) {
					$fail = API::History()->get(array(
						'output' => API_OUTPUT_EXTEND,
						'itemids' => $item['itemid'],
						'time_from' => $start_time,
	...
	case RSM_SLV_DNS_UDP_UPD_PFAILED:
		if (getRequest('type') == RSM_SLA_SCREEN_TYPE_GRAPH_1 || getRequest('type') == RSM_SLA_SCREEN_TYPE_GRAPH_2) {
			...
			// Failed tests and maximum expected number of tests per month.
			$item_keys = array(RSM_SLV_DNS_UDP_UPD_FAILED, RSM_SLV_DNS_UDP_UPD_MAX);
			$items = API::Item()->get(array(
				'output' => array('itemid', 'key_', 'value_type'),
				'hostids' => $data['tld']['hostid'],
				'filter' => array(
			...
			foreach ($items as $item) {
				if ($item['key_'] == RSM_SLV_DNS_UDP_UPD_FAILED) {
					$fail = API::History()->get(array(
						'output' => API_OUTPUT_EXTEND,
						'itemids' => $item['itemid'],
						'time_from' => $start_time,
	...
	case RSM_SLV_RDDS_UPD_PFAILED:
		if (getRequest('type') == RSM_SLA_SCREEN_TYPE_GRAPH_1 || getRequest('type') == RSM_SLA_SCREEN_TYPE_GRAPH_2) {
			if (getRequest('type') == RSM_SLA_SCREEN_TYPE_GRAPH_1) {
				$graph_name = RDDS_QUERY_RTT_GRAPH_1;
			}
			elseif (getRequest('type') == RSM_SLA_SCREEN_TYPE_GRAPH_2) {
				$graph_name = RDDS_QUERY_RTT_GRAPH_2;
			}
			elseif (getRequest('type') == RSM_SLA_SCREEN_TYPE_GRAPH_3) {
				$graph_name = RDDS_80_QUERY_RTT_GRAPH_1;
			}
			elseif (getRequest('type') == RSM_SLA_SCREEN_TYPE_GRAPH_5) {
			...
			// Failed tests and maximum expected number of tests per month.
			$item_keys = array(RSM_SLV_RDDS_UPD_FAILED, RSM_SLV_RDDS_UPD_MAX);
			$items = API::Item()->get(array(
				'output' => array('itemid', 'key_', 'value_type'),
				'hostids' => $data['tld']['hostid'],
				'filter' => array(
			...
			foreach ($items as $item) {
				if ($item['key_'] == RSM_SLV_RDDS_UPD_FAILED) {
					$fail = API::History()->get(array(
						'output' => API_OUTPUT_EXTEND,
						'itemids' => $item['itemid'],
						'time_from' => $start_time,
	...
	case RSM_SLV_EPP_RTT_LOGIN_PFAILED:
		if (getRequest('type') == RSM_SLA_SCREEN_TYPE_GRAPH_1 || getRequest('type') == RSM_SLA_SCREEN_TYPE_GRAPH_2) {
			...
			// Failed tests and maximum expected number of tests per month.
			$item_keys = array(RSM_SLV_EPP_RTT_LOGIN_FAILED, RSM_SLV_EPP_RTT_LOGIN_MAX);
			$items = API::Item()->get(array(
				'output' => array('itemid', 'key_', 'value_type'),
				'hostids' => $data['tld']['hostid'],
				'filter' => array(
			...
			foreach ($items as $item) {
				if ($item['key_'] == RSM_SLV_EPP_RTT_LOGIN_FAILED) {
					$fail = API::History()->get(array(
						'output' => API_OUTPUT_EXTEND,
						'itemids' => $item['itemid'],
						'time_from' => $start_time,
	...
	case RSM_SLV_EPP_RTT_INFO_PFAILED:
		if (getRequest('type') == RSM_SLA_SCREEN_TYPE_GRAPH_1) {
			...
			// Failed tests and maximum expected number of tests per month.
			$item_keys = array(RSM_SLV_EPP_RTT_INFO_FAILED, RSM_SLV_EPP_RTT_INFO_MAX);
			$items = API::Item()->get(array(
				'output' => array('itemid', 'key_', 'value_type'),
				'hostids' => $data['tld']['hostid'],
				'filter' => array(
			...
			foreach ($items as $item) {
				if ($item['key_'] == RSM_SLV_EPP_RTT_INFO_FAILED) {
					$fail = API::History()->get(array(
						'output' => API_OUTPUT_EXTEND,
						'itemids' => $item['itemid'],
						'time_from' => $start_time,
	...
	case RSM_SLV_EPP_RTT_UPDATE_PFAILED:
		if (getRequest('type') == RSM_SLA_SCREEN_TYPE_GRAPH_1 || getRequest('type') == RSM_SLA_SCREEN_TYPE_GRAPH_2) {
			...
			// Failed tests and maximum expected number of tests per month.
			$item_keys = array(RSM_SLV_EPP_RTT_UPDATE_FAILED, RSM_SLV_EPP_RTT_UPDATE_MAX);
			$items = API::Item()->get(array(
				'output' => array('itemid', 'key_', 'value_type'),
				'hostids' => $data['tld']['hostid'],
				'filter' => array(
			...
			foreach ($items as $item) {
				if ($item['key_'] == RSM_SLV_EPP_RTT_UPDATE_FAILED) {
					$fail = API::History()->get(array(
						'output' => API_OUTPUT_EXTEND,
						'itemids' => $item['itemid'],
						'time_from' => $start_time,
