...
sub db_connect
{
	...
	$_global_sql = 'DBI:mysql:'.$section->{'db_name'}.':'.$section->{'db_host'};
...
sub get_probe_times
{
	...
	{
		wrn("OBSOLETED: Probe main status items (".PROBE_KEY_ONLINE.") not available, will calculate manually.");

		my $start_time = time();

		$result = __get_probe_times_obsoleted($from, $till, $probes_ref);

		wrn("OBSOLETED: Probe main status manual calculation ", selected_period($from, $till), " took ", format_stats_time(time() - $start_time));
	...
	if (!defined($result))
	{
		wrn("OBSOLETED: Probe main status items (".PROBE_KEY_ONLINE.") have no values, will calculate manually.");
...
sub __get_probe_times_obsoleted
{
	my $from = shift;
	my $till = shift;
	my $probes_ref = shift; # { host => hostid, ... }
	...
	foreach my $probe (keys(%$probes_ref))
	{
		...
		my $times_ref = __get_lastaccess_times_obsoleted($itemid, $probe_avail_limit, $from, $till);
		...
		if (scalar(@$times_ref) != 0)
		{
			...
			$times_ref = __get_probestatus_times_obsoleted($probe, $hostid, $times_ref, PROBE_KEY_MANUAL);
		}
		...
		if (scalar(@$times_ref) != 0)
		{
			...
			$times_ref = __get_probestatus_times_obsoleted($probe, $hostid, $times_ref, PROBE_KEY_AUTOMATIC);
		}
...
sub __get_lastaccess_times_obsoleted
{
	my $itemid = shift;
...
sub __get_probestatus_times_obsoleted
{
	my $probe = shift;
	my $hostid = shift;
