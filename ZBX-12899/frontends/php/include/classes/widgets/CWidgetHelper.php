...
class CWidgetHelper {
	...
	public static function getNavTree($field) {
		...
		foreach ($navtree_items as $i => $navtree_item) {
			$navtree[] = new CVar($field_name.'['.$i.'][name]', $navtree_item['name']);
			$navtree[] = new CVar($field_name.'['.$i.'][order]', $navtree_item['order']);
			$navtree[] = new CVar($field_name.'['.$i.'][parent]', $navtree_item['parent']);
			$navtree[] = new CVar($field_name.'['.$i.'][sysmapid]', array_key_exists('sysmapid', $navtree_item)
